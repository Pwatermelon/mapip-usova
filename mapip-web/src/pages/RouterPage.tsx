  ...
  const build = async () => {
    ...
    try {
      ...
      const fc: GeoJSON.FeatureCollection = {
        ...
        features:
          coords.length >= 2
            ? [{ type: "Feature", geometry: { type: "LineString", coordinates: coords.map(([lat, lon]) => [lon, lat]) }, properties: {} }]
            : [],
      };
      ...
      setMsg(`Маршрут: ~${km} км${min != null ? `, ~${min} мин` : ""}. Участки без данных об объектах можно подсветить на клиенте, сравнивая с ${objects.length} объектами из core API.`);
    } catch (e) {
      setErr(String(e));
    }
  };
